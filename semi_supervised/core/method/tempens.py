...
class TemporalEnsembling(BasicMethod):
    def __init__(self,
        ...
        self.loss_tempens = torch.nn.MSELoss(reduction='mean').cuda()

        self.training_targets = torch.zeros((len(train_loader.dataset), self.num_classes))
        self.ensemble_prediction = torch.zeros((len(train_loader.dataset), self.num_classes))
    ...
    def _train_one_epoch(self, epoch, cons_weight, init_mode=False):
        if not init_mode:
            ...
            epoch_predictions = torch.zeros((self.training_targets.size(0), self.num_classes))
        ...
        for i, (input_pack, target, idxes) in enumerate(self.train_loader):
            if len(input_pack) == 2:
                input, _ = input_pack
            else:
                input = input_pack
            ...
            try:
                ...
                tempens_target_var = torch.autograd.Variable(self.training_targets[idxes].cuda(), requires_grad=False)
            except:
                target_var = torch.autograd.Variable(target)
                tempens_target_var = torch.autograd.Variable(self.training_targets[idxes])
            ...
            predictions = F.softmax(output_1, dim=1)

            loss_ce = self.loss_ce(output_1, target_var) / minibatch_size
            loss_tempens = cons_weight * self.loss_tempens(predictions, tempens_target_var)

            loss = loss_ce + loss_tempens
            losses_all.update(loss.item())
            super(TemporalEnsembling, self).log_to_tf("supervised_loss", loss_ce.item(), self.global_step, True)
            super(TemporalEnsembling, self).log_to_tf("consistency_loss", loss_tempens.item(), self.global_step, True)
            ...
            losses_pi.update(loss_tempens.item())
            ...
            for i, j in enumerate(idxes):
                epoch_predictions[j] = predictions[i].data.clone()  # Gather epoch predictions.
        ...
        if not init_mode:
            ...
            self.ensemble_prediction = (self.args.prediction_decay * self.ensemble_prediction) + (
                        1.0 - self.args.prediction_decay) * epoch_predictions
